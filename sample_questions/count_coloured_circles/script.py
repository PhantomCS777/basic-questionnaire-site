...
def create_images(num_images, num_objects_list , file):
    ...
    for num_objects in num_objects_list:
        ...
        for img_index in range(num_images):
            ...
            radii = [random.randint(10, 20) for _ in range(max_obj)]
            ...
            # Draw the circles
            red_circle = num_objects
            indices = random.sample(range(0, max_obj), num_objects)
            for i, (x, y, radius) in enumerate(positions):
                color = ""
                if i in indices:
                    color = "red"
                # draw.ellipse([x - radius, y - radius, x + radius, y + radius], fill="black")
                #draw a random colored circle from red blue green yellow
                else:
