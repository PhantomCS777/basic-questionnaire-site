    ...
    for num_objects in num_objects_list:
        ...
        for img_index in range(num_images):
            ...
            for i in range (num_objects):
                ...
                if shape_type == 'circle':
                    shape = plt.Circle((x, y), radius, ec="blue", fill=False)
                elif shape_type == 'triangle':
                    triangle = np.array([[x, y + radius], [x - radius, y - radius], [x + radius, y - radius]])
                    shape = plt.Polygon(triangle, ec="blue", fill = False)
                elif shape_type == 'rectangle':
                    shape = plt.Rectangle((x - radius, y - radius), 2 * radius, 2 * radius, ec="blue", fill=False)
                elif shape_type == 'pentagon':
                    pentagon = np.array([
                        [x, y + radius], [x + 0.95 * radius, y + 0.31 * radius],
                        [x + 0.59 * radius, y - 0.81 * radius], [x - 0.59 * radius, y - 0.81 * radius],
                    ...
                    shape = plt.Polygon(pentagon, ec="blue", fill=False)
                ax.add_patch(shape)
